...
def loadData(filename):
    ...
    fullrec = [word for word in fullrec if word not in stop_words]
    ...
    for key, value in origcounts.items():
        if value >= min_count:
            fullrec_filtered.append(key)
        else:
            fullrec_filtered.append('UNK')
    
    for key, value in origcounts.items():
        if key in fullrec_filtered:
            wordcounts[key] = value
        if key == 'UNK':
            pass
    
    unk_counter = []
    for unk in fullrec_filtered:
        if unk == 'UNK':
            unk_counter.append(unk)
    
    unk_dict = Counter(unk_counter)
    
    wordcounts.update(unk_dict)
    
            
    #... after filling in fullrec_filtered, replace the original fullrec with this one.
    fullrec = fullrec_filtered
#    global uniqueCounts
#    uniqueCounts = Counter(fullrec)
...
#    global wordcounts
    ...
    exp_count_array = [word * math.exp(exp_power) for word in count_list]
    ...
    prob_dist = [word / max_exp_count for word in exp_count_array]
    ...
    table_size = 1e7
    num_iterations = [prob * table_size for prob in prob_dist]
    count = 0
    for i in range(len(uniqueWords)):
        for j in range(int(num_iterations[i])):
            cumulative_dict[count] = i
            count += 1
...
def generateSamples(context_idx, num_samples):
    ...
    randomselection = randint(0, len(samplingTable)-1)
    for i in range(num_samples):
        if randomselection != context_idx:
            random_indice = samplingTable[randomselection]
            results.append(random_indice)
    return results
...
def performDescent(num_samples, learning_rate, center_token, context_words,W1,W2,negative_indices):
    ...
    #output layer
    W2_sig = sigmoid(np.dot(np.transpose(W2[context_words]), h))
    W2[context_words] = W2[context_words] - learning_rate * (W2_sig - 1) * h
    
    for i in negative_indices:
        W2[i] = W2[i] - learning_rate * (W2_sig - 0) * h
    
    
    #input layer
    total_w1 = []
    W1_sig = sigmoid(np.dot(np.transpose(W1[center_token]), h))
    W1_summed_up = (W1_sig - 1) * W1[center_token]
    for i in negative_indices:
        W1_summed_up += (W1_sig - 0) * W1[i]

    W1[center_token] = W1[center_token] - learning_rate*(W1_summed_up)
    
    #negative likelihood
#    neg_likely = []
#    for i in negative_indices:
#        neg_likely.append(math.log(sigmoid(-1 * np.dot(W2[i], h))))
#    neg_likely_summed = np.sum(neg_likely)
#    nll_new = -1 * (math.log(sigmoid(np.dot(W2[context_words].T, h)))) - neg_likely_summed
#
#
#    return [nll_new]
    return 0
    ...
    for j in range(0,epochs):
        ...
        for i in range(start_point,end_point):
            ...
            for k in range(0, len(context_window)):
                ...
                nll = performDescent(num_samples, learning_rate, center_token, context_words,W1,W2,negative_indices)
